...
Use the local Everything CLI installed at `.claude\skills\everything\bin\es.exe` for fast file and folder searches before broad recursive scans.

Examples:

```powershell
& '.claude\skills\everything\bin\es.exe' -get-everything-version
& '.claude\skills\everything\bin\es.exe' -n 50 '*.sln'
& '.claude\skills\everything\bin\es.exe' -n 200 'vcvars64.bat'
```
